...
# Point working directory to GPA paper folder depending on OS
...
teacher <- readxl::read_excel("./data/04_gpa_in.xlsx", sheet = "STP")
#   Sheet 2 "SKR" (written exam grades)
written <- readxl::read_excel("./data/04_gpa_in.xlsx", sheet = "SKR")
#   Sheet 3 "MUN" (oral exam grades)
oral <- readxl::read_excel("./data/04_gpa_in.xlsx", sheet = "MUN")
if (interactive()) {
    names(teacher)
} else {
...
if (interactive()) {
    rbind(
        dim(teacher_gpa), dim(written_gpa), dim(oral_gpa)
    )
}
...
foreach (i = 1:n_iter) %dopar% {
    ...
    subj[i, 3] <- max(stp[i, 3], stp[i, 4], na.rm = TRUE)
}
...
foreach (i = 1:n_iter) %dopar% {
    pb$tick()
    subj[i, 6] <- max(stp[i, 7], stp[i, 8], na.rm = TRUE)
}
cat("\n")
...
foreach (i = 1:n_iter) %dopar% {
    pb$tick()
    subj[i, 7] <- max(stp[i, 9], stp[i, 10], na.rm = TRUE)
}
cat("\n")
...
foreach (i = 1:n_iter) %dopar% {
    pb$tick()
    subj[i, 8] <- max(stp[i, 11], stp[i, 12], na.rm = TRUE)
}
cat("\n")
...
foreach (i = 1:n_iter) %dopar% {
    pb$tick()
    subj[i, 9] <- max(stp[i, 13], stp[i, 14], na.rm = TRUE)
}
cat("\n")
...
foreach (i = 1:n_iter) %dopar% {
    pb$tick()
    subj[i, 10] <- max(stp[i, 15], stp[i, 16], na.rm = TRUE)
}
cat("\n")
...
foreach (i = 1:n_iter) %dopar% {
    pb$tick()
    subj[i, 11] <- max(stp[i, 17], stp[i, 18], na.rm = TRUE)
}
cat("\n")
...
foreach (i = 1:n_iter) %dopar% {
    pb$tick()
    subj[i, 12] <- max(stp[i, 19], stp[i, 20], na.rm = TRUE)
}
cat("\n")
...
# Save teacher_final
data.table::fwrite(gpa_final, "./data/04_gpa_out.csv", row.names = FALSE)
