...
###Splitting the group variable, getting variables: gender and language.

Group1=RTdata[,c("ID","Group")]

Group2<- cbind(Group1[-3],                   t(as.data.frame(strsplit(Group1$Group, '_'),                                                row.names=c("Group1", "Group2","Group3"))), row.names=NULL)

TOTGRUPPE<-Group2 %>% separate(Group2,into = c("Gender","Language"),sep = -2,convert = TRUE)
...
write.table(DONEASSIGNMENT2,file = "data/DONEASSIGNMENT2-exported.txt",row.names = TRUE,sep = "\t",col.names = NA)
